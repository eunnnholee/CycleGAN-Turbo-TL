...
# kernel path: /tmp/torchinductor_elicer/iy/ciyiaay3pw34gcgo3qtiwuvx6hjwubnkel4gro4uxv2f4jzszvkd.py
...
triton_per_fused__to_copy_convolution_backward_div_mul_sigmoid_sub_1 = async_compile.triton('triton_per_fused__to_copy_convolution_backward_div_mul_sigmoid_sub_1', '''
import triton
import triton.language as tl
from triton.compiler.compiler import AttrsDescriptor
...
#   %where_3 : [num_users=2] = call_function[target=torch.ops.aten.where.self](args = (%gt_3, %mm_1, %mul_118), kwargs = {})
...
def call(args):
    ...
    with torch.cuda._DeviceGuard(0):
        ...
        # Topologically Sorted Source Nodes: [target_], Original ATen: [aten.div, aten.sigmoid, aten._to_copy, aten.sub, aten.mul, aten.convolution_backward]
        stream0 = get_raw_stream(0)
        triton_per_fused__to_copy_convolution_backward_div_mul_sigmoid_sub_1.run(buf12, tangents_1, buf13, 1, 18, grid=grid(1), stream=stream0)
        ...
        stream0 = get_raw_stream(0)
        triton_per_fused__to_copy_convolution_backward_div_mul_sigmoid_sub_1.run(buf28, tangents_1, buf29, 1, 18, grid=grid(1), stream=stream0)
        del tangents_1
